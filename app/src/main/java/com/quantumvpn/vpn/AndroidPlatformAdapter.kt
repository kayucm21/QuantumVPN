...
internal class AndroidPlatformAdapter(
    ...
    private val blockNonVpnTraffic: Boolean = false,
    ...
    override fun openTun(options: TunOptions): Int {
        ...
        if (scopeMode == AppScopeMode.All) {
            check(includePackages.isEmpty() && excludePackages.isEmpty()) {
                "Полный VPN не допускает include_package/exclude_package."
            }
            check(expectedPackages.isEmpty()) {
                "Полный VPN ожидает пустой Android scope."
            }
        } else {
            val actualPackages = if (scopeMode == AppScopeMode.Include || scopeMode == AppScopeMode.Block) {
                includePackages
            } else {
                excludePackages
            }
            check(
                if (scopeMode == AppScopeMode.Include || scopeMode == AppScopeMode.Block) {
                    excludePackages.isEmpty()
                } else {
                    includePackages.isEmpty()
                },
        ...
        val builder = service.Builder()
            ...
            .setMtu(options.mtu)
            .setBlocking(blockNonVpnTraffic)
    ...
    override fun getInterfaces(): NetworkInterfaceIterator {
        ...
        val result = connectivity.networksCompat.mapNotNull { network ->
