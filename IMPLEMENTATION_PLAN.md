# QuantumVPN Android — план реализации
...
| Ядро | `sing-box-extended` `v1.13.18-extended-2.6.5` |
| Commit ядра | `e8f6936480b7fa9738911e3e7fc2ec0d8a634a88` |
| Модули | Только `app` |
...
- [x] `I0-04` Создать `QuantumVpnApplication`, ручной `AppContainer`, `MainActivity` и четыре нижние вкладки-заглушки.
...
- [x] `I2-05` Внутренне включать package QuantumVPN для health-check, не показывая его как пользовательский выбор.
...
- [ ] `R8-12` Невыбранное приложение передаёт фиксированный объём без пропорционального CPU/network роста QuantumVPN. На AVD 8 MiB дали median TUN=0; физический OEM-повтор открыт.
...
- [x] Невыбранный трафик не проходит через TUN/libbox: 5 × 8 MiB дали 0 UID RX+TX
  QuantumVPN и median 0 TUN bytes (единственный интерфейсный шум — 96 bytes).
