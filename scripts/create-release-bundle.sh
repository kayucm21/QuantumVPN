...
printf '%s\n' "${ARTIFACTS[@]}" | jq -s \
    ...
    --arg application_id "com.quantumvpn" \
...
{
    printf '%s\n' \
        "## QuantumVPN $ZAPRET_VERSION_NAME" \
        '' \
        "- Канал: $ZAPRET_RELEASE_CHANNEL" \
        '- Android package: com.quantumvpn' \
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Исправлен общий TUN-слой userspace WireGuard: внешний Android TUN получает min(1500, endpoint MTU), а не Android-default 9000; для endpoint 1280 итоговый TUN тоже 1280.' \
        '- MTU применяется одинаково во всех четырёх DNS-режимах и только к runtime-копии; сохранённый JSON не переписывается.' \
