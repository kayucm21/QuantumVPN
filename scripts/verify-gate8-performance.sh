...
COMPONENT="$PACKAGE/com.quantumvpn.debug.Gate8PerformanceProbeReceiver"
CONTROL_URI="content://com.quantumvpn.debug.test.traffic"
...
cleanup() {
    ...
    probe "com.quantumvpn.debug.GATE8_PERF_CLEANUP" >/dev/null 2>&1 || true
...
adb_call shell wm dismiss-keyguard >/dev/null 2>&1 || true
adb_call shell am start -W -n "$PACKAGE/com.quantumvpn.MainActivity" >/dev/null
sleep 1

pid_of_app() {
    local pid
    pid="$(adb_call shell pidof "$PACKAGE" | tr -d '\r' | awk '{print $1}')"
    [[ "$pid" =~ ^[0-9]+$ ]] || { echo "QuantumVPN process is not running" >&2; exit 1; }
    printf '%s\n' "$pid"
}
...
prepare() {
    ...
    adb_call shell am start -W -n "$PACKAGE/com.quantumvpn.MainActivity" >/dev/null
    sleep 1
    probe "com.quantumvpn.debug.GATE8_PERF_PREPARE" \
        --es route_mode "$route" \
    ...
    connected="$(probe "com.quantumvpn.debug.GATE8_PERF_CONNECT")"
...
stop_vpn() {
    local stopped
    stopped="$(probe "com.quantumvpn.debug.GATE8_PERF_STOP")"
...
selected_workload() {
    ...
    while (( remaining > 0 )); do
        ...
        output="$(probe "com.quantumvpn.debug.GATE8_PERF_TRAFFIC" \
...
dns_workload() {
    local prefix="$1"
    probe "com.quantumvpn.debug.GATE8_PERF_DNS" \
...
measure_once() {
    ...
    pid="$(pid_of_app)"
    before_status="$(probe "com.quantumvpn.debug.GATE8_PERF_STATUS")"
    ...
    runtime_status="$(probe "com.quantumvpn.debug.GATE8_PERF_STATUS")"
...
probe "com.quantumvpn.debug.GATE8_PERF_CLEANUP" >/dev/null
adb_call shell input keyevent KEYCODE_HOME >/dev/null
...
adb_call shell input keyevent KEYCODE_HOME >/dev/null
probe "com.quantumvpn.debug.GATE8_PERF_VISIBILITY" --ez home_visible false --ez diagnostics_visible false >/dev/null
run_repeats "ui_closed" idle
adb_call shell am start -W -n "$PACKAGE/com.quantumvpn.MainActivity" >/dev/null
tap_text "Настройки"
tap_text "Диагностика" true
diagnostics_status="$(probe "com.quantumvpn.debug.GATE8_PERF_STATUS")"
