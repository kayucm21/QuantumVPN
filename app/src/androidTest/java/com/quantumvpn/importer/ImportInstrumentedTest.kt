...
class ImportInstrumentedTest {
    ...
    fun selectedDocumentIsReadThroughContentResolver() {
        ...
        assertTrue(ImportParser.parse(raw, ProfileSource.File).candidate is ImportCandidate.RawJson)
    }
    ...
    fun clipboardIsReadOnlyAfterExplicitReaderCall() {
        ...
        assertTrue(ImportParser.parse(imported, ProfileSource.Clipboard).candidate is ImportCandidate.RawJson)
    ...
    fun newManagedProfilePreviewDefaultsToRussiaDirect() = runBlocking {
        ...
        try {
            val application = context.applicationContext as QuantumVpnApplication
            ...
            val viewModel = ViewModelProvider(
                ...
                ProfilesViewModel.Factory(
                    ...
                    happRoutingStore = application.container.happRoutingProfileStore,
                    recentServersStore = application.container.recentServersStore,
                    favoriteServersStore = application.container.favoriteServersStore,
                    profileBackupImporter = application.container.profileBackupImporter,
                    eventJournal = application.container.eventJournalStore,
                    sessionSwitchHistory = application.container.sessionSwitchHistoryStore,
                    subscriptionQuotaStore = application.container.subscriptionQuotaStore,
                    disconnectReasonStore = application.container.disconnectReasonStore,
                    speedTestHistoryStore = application.container.speedTestHistoryStore,
                    connectDurationStore = application.container.connectDurationStore,
                    importClipboardClear = {
                        application.container.importReader.clearClipboardSafely()
                    },
                ),
    ...
    fun allShareLinkProtocolsParseToConfigAcceptedByPinnedLibbox() {
        ...
        val candidate = ImportParser.parse(links, ProfileSource.Clipboard).candidate as ImportCandidate.Managed
    ...
    fun wireguardAndAmneziaWg2ConfigsPassPinnedLibboxCheck() {
        val wireGuard = ImportParser.parse(WIREGUARD_CONF, ProfileSource.File).candidate as ImportCandidate.WireGuard
        val amnezia = ImportParser.parse(AWG2_CONF, ProfileSource.File).candidate as ImportCandidate.WireGuard
