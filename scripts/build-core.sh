...
git -C "$SOURCE_DIR" fetch --force --depth=1 origin "refs/tags/$CORE_TAG:refs/tags/$CORE_TAG" || \
    git -C "$SOURCE_DIR" fetch --force --depth=1 origin "refs/heads/$CORE_TAG:refs/heads/$CORE_TAG"
...
if git -C "$SOURCE_DIR" rev-parse --verify -q "refs/tags/${CORE_TAG}^{commit}" >/dev/null 2>&1; then
    TAG_COMMIT="$(git -C "$SOURCE_DIR" rev-parse "refs/tags/${CORE_TAG}^{commit}")"
elif git -C "$SOURCE_DIR" rev-parse --verify -q "refs/heads/${CORE_TAG}^{commit}" >/dev/null 2>&1; then
    TAG_COMMIT="$(git -C "$SOURCE_DIR" rev-parse "refs/heads/${CORE_TAG}^{commit}")"
else
    echo "Core ref not found: $CORE_TAG" >&2
    exit 1
fi
if [[ "$TAG_COMMIT" != "$CORE_COMMIT" ]]; then
    echo "Core ref mismatch: $CORE_TAG points to $TAG_COMMIT" >&2
    exit 1
fi
...
if [[ "$(go env GOOS)" == "windows" ]]; then
    # Windows often has localhost commented out in System32\drivers\etc\hosts,
    # so dns/transport/hosts TestHosts is not portable here.
    mapfile -t DNS_TEST_PACKAGES < <(
        go list ./dns/... ./route/rule ./experimental/libbox |
            grep -v '/dns/transport/hosts$'
    )
    go test "${DNS_TEST_PACKAGES[@]}"
    # Host wireguard unit tests need Linux/Android-style netFD linknames.
    echo "Skipping host wireguard package tests on windows"
else
