...
COMPONENT="$PACKAGE/com.quantumvpn.debug.Gate7UpgradeProbeReceiver"
...
seed="$(probe com.quantumvpn.debug.GATE7_SEED)"
...
"$ADB" shell am start -W -n "$PACKAGE/com.quantumvpn.MainActivity" >/dev/null
upgraded="$(probe com.quantumvpn.debug.GATE7_STATUS)"
...
keytool -genkeypair -noprompt \
    ...
    -dname "CN=QuantumVPN incompatible test" >/dev/null 2>&1
"$APKSIGNER" sign \
...
final="$(probe com.quantumvpn.debug.GATE7_STATUS)"
for field in version=701002 profiles=1 active=true theme=Dark channel=Beta apps=1; do
