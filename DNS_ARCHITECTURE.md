# ADR-002: DNS и изоляция приложений в QuantumVPN
...
| Проверенный исходник | tag `v1.13.18-extended-2.6.5`, commit `e8f6936480b7fa9738911e3e7fc2ec0d8a634a88` |
...
- **политика QuantumVPN** — обязательное решение нашего приложения поверх этих контрактов.
...
| Проверено | Подтверждённое поведение | Следствие для QuantumVPN |
|---|---|---|
...
Pinned Android gitlink фиксирует полезный пример поведения, но не доказывает совместимость с ABI нашего libbox. В проверенном core API менялся после commit Android reference, а upstream Android workflow для stable/testing дополнительно переключает submodule на плавающие `main`/`dev`. Поэтому QuantumVPN компилирует собственный platform adapter против AAR, собранного в том же CI из pinned core commit; reference-код не копируется как готовый бинарно-совместимый слой.

В ходе аудита также обнаружено, что upstream release asset с именем `1.13.14-extended-2.5.2` содержит revision `ea48501b1d078cbf55437a8d37987b2feb842796`, а не commit тега `ff11f00…`. На дату аудита между ними в DNS/libbox/route-коде различий нет — изменён только `README.md` — поэтому проверенные JSON совместимы с обоими. Однако upstream workflow принудительно создаёт локальный tag и умеет заменять assets существующего release. Следовательно, имя release не является доказательством происхождения бинарника: QuantumVPN собирает libbox и проверочный CLI самостоятельно только из полного pinned commit.
...
6. После проверки пользовательского списка внутренне добавляем пакет QuantumVPN. Это позволяет выполнять DNS/HTTP health-check через VPN; остальные служебные запросы явно привязываются к VPN либо underlying network согласно операции.
...
- пакет QuantumVPN добавляется внутренне и не отображается как пользовательский выбор; системные приложения автоматически не добавляются;
...
Общий cache key безопасен для сгенерированных режимов только при инварианте: один и тот же домен в пределах активной конфигурации всегда выбирает один и тот же DNS-транспорт. QuantumVPN не создаёт DNS-выбор одного домена, зависящий от пакета/UID. При смене DNS-режима или правил выполняется перезапуск core. Если пользователь вручную строит контекстно-зависимую схему, это режим «Из JSON», и он сам решает, включать ли `independent_cache`.
...
- [Проверенный commit sing-box-extended](https://github.com/shtorm-7/sing-box-extended/tree/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88)
- [Pinned source: libbox TunOptions](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/experimental/libbox/tun.go)
- [Pinned source: platform DNS](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/experimental/libbox/dns.go)
- [Pinned source: fallback strategies](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/dns/transport/fallback/strategy.go)
- [Pinned source: DNS client/cache](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/dns/client.go)
- [Pinned source: DNS router/reset/reverse mapping](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/dns/router.go)
- [Pinned source: libbox package override](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/daemon/instance.go)
- [Pinned source: Android owner lookup activation](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/route/router.go)
- [Pinned source: owner lookup before route matching](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/route/route.go)
- [Pinned source: protected/default dialer](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/common/dialer/default.go)
- [Pinned source: libbox platform wrapper](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/experimental/libbox/service.go)
- [Pinned source: TUN inbound lifecycle](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/protocol/tun/inbound.go)
- [Pinned source: release workflow](https://github.com/shtorm-7/sing-box-extended/blob/e8f6936480b7fa9738911e3e7fc2ec0d8a634a88/.github/workflows/build.yml)
