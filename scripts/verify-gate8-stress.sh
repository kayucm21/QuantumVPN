...
COMPONENT="$PACKAGE/com.quantumvpn.debug.Gate8StressProbeReceiver"
...
$ADB shell am start -W -n "$PACKAGE/com.quantumvpn.MainActivity" >/dev/null
...
cleanup() {
    ...
    probe "com.quantumvpn.debug.GATE8_CLEANUP" >/dev/null 2>&1 || true
    $ADB shell appops set "$PACKAGE" ACTIVATE_VPN default >/dev/null 2>&1 || true
}
trap cleanup EXIT

prepared="$(probe "com.quantumvpn.debug.GATE8_PREPARE")"
...
for ((cycle = 1; cycle <= CONNECT_CYCLES; cycle++)); do
    connected="$(probe "com.quantumvpn.debug.GATE8_CONNECT")"
    if [[ ";$connected;" != *";state=connected;"* || ";$connected;" != *";tun=1;"* ]]; then
        echo "Connect cycle $cycle failed: $connected" >&2
        exit 1
    fi
    stopped="$(probe "com.quantumvpn.debug.GATE8_STOP")"
    assert_idle "$stopped"
...
after_cycles="$(probe "com.quantumvpn.debug.GATE8_STATUS")"
...
for ((transition = 1; transition <= NETWORK_TRANSITIONS; transition++)); do
    probe "com.quantumvpn.debug.GATE8_ARM_RESTART" >/dev/null
    ...
    while (( SECONDS < deadline )); do
        candidate="$(probe "com.quantumvpn.debug.GATE8_STATUS")"
...
final="$(probe "com.quantumvpn.debug.GATE8_STOP")"
assert_idle "$final"
