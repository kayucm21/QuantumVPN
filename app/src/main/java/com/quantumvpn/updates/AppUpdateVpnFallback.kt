...
class AndroidUpdateInstallIntentFactory(context: Context) : UpdateInstallIntentFactory {
    ...
    override fun create(file: java.io.File): Intent {
        ...
        return Intent(Intent.ACTION_INSTALL_PACKAGE).apply {
            ...
            clipData = ClipData.newRawUri("QuantumVPN update", uri)
...
/** Integrates the isolated updater module with underlying-network bind + optional VPN route. */
class AppUpdateVpnFallback(
    ...
    init {
        UpdateSocketProtection.binder = UpdateSocketBinder { socket ->
            bindSocketToUnderlying(socket)
        }
    }

    override suspend fun <T> withUnderlyingNetwork(block: () -> T): T {
        val underlying = findUnderlyingNetwork()
        if (underlying == null) return block()
        val previous = runCatching { connectivity.boundNetworkForProcess }.getOrNull()
        return try {
            connectivity.bindProcessToNetwork(underlying)
            block()
        } finally {
            runCatching { connectivity.bindProcessToNetwork(previous) }
        }
    }
    ...
    private fun bindSocketToUnderlying(socket: Socket): Boolean {
    ...
    private fun findUnderlyingNetwork(): Network? {
        val networks = connectivity.networksCompat
