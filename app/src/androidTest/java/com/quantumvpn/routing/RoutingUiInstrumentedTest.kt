...
class RoutingUiInstrumentedTest {
    ...
    @Test
    fun routingScreenKeepsTrafficControlsWithoutJsonEditor() {
        composeRule.onNodeWithText("Маршруты").performClick()
        composeRule.waitUntil(20_000) {
            runCatching {
                composeRule.onNodeWithText("Правило трафика").fetchSemanticsNode()
            }.isSuccess
        }
        composeRule.onNodeWithText("Правило трафика").assertExists()
        composeRule.onNodeWithText("Расширенный JSON").assertDoesNotExist()
    }
